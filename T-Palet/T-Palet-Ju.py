...
sys.path.append('/Users/deikazuki/T-Palet')
import TObject as t
import Speaker as s
import MyException
...
def profile(ref):
    #
    accept = False
    # askArtValでユーザーが入力に関してNoと答えたときにループしてYesの時には抜け出す処理
    while not accept:
        # print("loop_start_accept:",accept)
        sp.speak(ref.surf+'さんについて、教えてください。例えば、「身長は170ｃｍです」みたいに。')
        rep = sp.askAtrVal('「終わり」で終了です。')
        if rep != False:
            accept = True
            # print("rep->notFalse_accept:",accept)
            continue
        elif rep == False:
            # print("rep->False_accept:",accept)
            continue
